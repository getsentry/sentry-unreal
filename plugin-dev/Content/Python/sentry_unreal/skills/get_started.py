...
## Step 1 - Check for Sentry's tools
...
whether Sentry's tools are in front of you right now.
...
Say that Sentry's tools aren't available, that the rest will run in the reduced form Steps 2 and 3
describe, and that connecting Sentry beforehand makes those steps automatic next time. Then carry on
to Step 2.
...
Without Sentry's tools, ask the user for the DSN. It is public and grants no access.
...
Without Sentry's tools, say plainly that arrival cannot be confirmed from here and tell the user
what to look for.
...
and issue URL shown to the user, and the test issue has been offered for cleanup. Without Sentry's
tools: the DSN is set, the SDK reports itself enabled, and the user has been told plainly what to
look for and that arrival wasn't confirmed from here.
"""
...
class SentryGetStartedSkill(unreal.AgentSkill):
    ...
    configures the DSN, and confirms a test event arrives in Sentry. Use when Sentry is installed
    but not configured, when the DSN needs to change, or when a configured setup isn't producing
    events."""
