...
{
	UserFeedback = sentry_value_new_object();
	sentry_value_set_by_key(UserFeedback, "message", sentry_value_new_string(TCHAR_TO_UTF8(*message)));
}
...
{
	sentry_value_set_by_key(UserFeedback, "contact_email", sentry_value_new_string(TCHAR_TO_UTF8(*email)));
}

FString FGenericPlatformSentryFeedback::GetContactEmail() const
{
	sentry_value_t email = sentry_value_get_by_key(UserFeedback, "contact_email");
...
void FGenericPlatformSentryFeedback::SetAssociatedEvent(const FString& eventId)
{
	...
	sentry_value_set_by_key(UserFeedback, "associated_event_id", sentry_value_new_string(TCHAR_TO_UTF8(*SanitizedEventId)));
}

FString FGenericPlatformSentryFeedback::GetAssociatedEvent() const
{
	sentry_value_t comment = sentry_value_get_by_key(UserFeedback, "associated_event_id");
