...
{
	UserFeedback = sentry_value_new_object();
	sentry_value_set_by_key(UserFeedback, "associated_event_id", sentry_value_new_string(TCHAR_TO_ANSI(*eventId->ToString())));
}
...
{
	sentry_value_set_by_key(UserFeedback, "contact_email", sentry_value_new_string(TCHAR_TO_ANSI(*email)));
}
...
{
	sentry_value_set_by_key(UserFeedback, "message", sentry_value_new_string(TCHAR_TO_UTF8(*comment)));
}

FString FGenericPlatformSentryUserFeedback::GetComment() const
{
	sentry_value_t comment = sentry_value_get_by_key(UserFeedback, "message");
