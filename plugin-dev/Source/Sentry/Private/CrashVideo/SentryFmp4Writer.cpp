...
namespace SentryFmp4Detail
{
	...
	static TArray<uint8> BuildMvhd()
	{
		...
		// Identity matrix
		WriteU32(P, 0x00010000); WriteU32(P, 0); WriteU32(P, 0);
		WriteU32(P, 0); WriteU32(P, 0x00010000); WriteU32(P, 0);
		WriteU32(P, 0); WriteU32(P, 0); WriteU32(P, 0x40000000);
	...
	static TArray<uint8> BuildVmhd()
	{
		...
		WriteU16(P, 0); WriteU16(P, 0); WriteU16(P, 0);    // opcolor (3 shorts)
...
TArray<TArray<uint8>> FSentryFmp4Writer::SplitAnnexB(const uint8* Data, int64 Size)
{
	...
	{
		int64 StartLen = 0;
		if (IsStart4(Data, i, Size)) { StartLen = 4; }
		else if (IsStart3(Data, i, Size)) { StartLen = 3; }
