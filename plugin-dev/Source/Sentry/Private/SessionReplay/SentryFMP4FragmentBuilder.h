 ...
 *
 * Every encoder backend produces Annex-B access units and needs the exact same
 ...
 */
class FSentryFMP4FragmentBuilder
{
public:
	explicit FSentryFMP4FragmentBuilder(FSentrySessionReplayRecorder& InRecorder);
	 ...
	 * the rates replay records at, where it keeps the most recent tenth of a
	 * second or more off disk - and a crash shortly after the first frame produces
	 * no clip at all. Backends that make every frame an IDR should enable this.
	 */
	void SetFlushEveryKeyframe(bool bInFlushEveryKeyframe) { bFlushEveryKeyframe = bInFlushEveryKeyframe; }

	/**
	 * Feeds one encoded access unit - the NAL units making up a single picture.
	 *
	 * @param AnnexB       one frame's NAL units as an Annex-B byte stream
