...
BeforeAll {
    ...
    # Connect to SauceLabs (reads all configuration from environment variables)
    ...
    # $global:SauceLabsCrashResult = Invoke-DeviceApp -ExecutablePath $script:ActivityName -Arguments $cmdlineCrashArgs
    ...
    $global:SauceLabsMessageResult = Invoke-DeviceApp -ExecutablePath $script:ActivityName -Arguments $cmdlineMessageArgs
...
AfterAll {
    # Disconnect from SauceLabs device (cleans up session)
    Write-Host "Disconnecting from SauceLabs device..." -ForegroundColor Yellow
    Disconnect-Device
