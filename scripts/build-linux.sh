...
chmod +x ./dockcross
./dockcross bash -c "cmake -S '${sentryNativeRoot}' -B '${sentryNativeRoot}/build' -D SENTRY_BACKEND=crashpad -D SENTRY_SDK_NAME=sentry.native.unreal -D CMAKE_BUILD_TYPE=RelWithDebInfo"
./dockcross bash -c "cmake --build '${sentryNativeRoot}/build' --target sentry --parallel"
